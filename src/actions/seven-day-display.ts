...
@action({ UUID: "com.claudegauge.usage.seven-day" })
export class SevenDayDisplayAction extends SingletonAction<ActionSettings> {
  private refreshManager = RefreshManager.getInstance();

  override onWillAppear(ev: WillAppearEvent<ActionSettings>): void {
    if (ev.action.isKey()) {
      this.refreshManager.registerAction(ev.action, "seven-day");
    }
  }
