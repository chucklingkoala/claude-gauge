...
@action({ UUID: "com.claudegauge.usage.five-hour" })
export class FiveHourDisplayAction extends SingletonAction<ActionSettings> {
  private refreshManager = RefreshManager.getInstance();

  override onWillAppear(ev: WillAppearEvent<ActionSettings>): void {
    if (ev.action.isKey()) {
      this.refreshManager.registerAction(ev.action, "five-hour");
    }
  }
